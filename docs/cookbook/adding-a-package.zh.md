...
manifest 的 `description` 就是 Web GUI 设置页插件清单展示的插件说明（由 `pnpm run verify-package-descriptions` 强制执行）：写一句简体中文，点明包的角色与所在能力接缝，至少 10 个字符且必须含有汉字。不带 `dsh` manifest 的库包则改为声明一句不少于 24 个字符、以字母或 `@` 开头的英文。插件卡片会逐字展示这一句，因此描述的是这个插件做什么——而不是它如何构建。
