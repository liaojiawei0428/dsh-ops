...
The manifest `description` is the plugin explanation the web GUI shows on its settings plugin inventory (`pnpm run verify-package-descriptions` enforces it): write one Simplified Chinese sentence naming the package role and its seam, at least 10 characters long and containing at least one Han character. Library packages without a `dsh` manifest instead declare one English sentence of at least 24 characters starting with a letter or `@`. The inventory card carries this line verbatim, so describe what the plugin does — not how it is built.
