...
当前 Cordis Loader 树的只读 Host 投影。`PluginInventoryGateway` 注册 `pluginInventory` 服务，并发布一个由 Typert 生成的直接 Remote：`pluginInventory/list`。每次调用都直接读取 `ctx.loader.entries()`，跳过结构性的 group 行，再按 Loader 顺序返回其余条目，并且只包含 Loader 条目 id、模块标识、manifest 描述、有效启用状态与当前根 Fiber 阶段。

描述即条目模块包清单中的 `description` 字段，通过每个 profile 启动行都会经过的已修复 `$DSH_HOME/profiles/node_modules` 回退路径读取。每个名称先探测 `<module>/package.json`，再探测所属包根，因此子路径行（例如 `@scope/pkg/startup`）也能报告所在包的文本；无法解析的名称（`cordis:` 内置、未安装行）以及没有可用文本的清单投影为 `null` 而不是使快照失败。
